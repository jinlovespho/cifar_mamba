...
class ViT_Orig(nn.Module):
    def __init__(self, in_c:int=3, num_classes:int=10, img_size:int=32, patch:int=8, dropout:float=0., num_layers:int=7, hidden:int=384, mlp_hidden:int=384*4, head:int=8, is_cls_token:bool=True, args=None):
        super().__init__()
        # hidden=384
    ...
    def forward(self, x):
        ...
        for i_th_layer, enc_layer in enumerate(self.enc):
            # if self.args.prune_layer == i_th_layer:
            #     out = enc_layer(out, do_prune=True, args=self.args)
            # else:
            #     out = enc_layer(out, do_prune=False)
            out = enc_layer(out, do_prune=False)
